...
//        urlQueue.put("https://www.zhihu.com/people/yao-cheng-46");
            urlQueue.put("https://www.zhihu.com/people/mo-dao-bie-chi");
...
//        System.out.println(userUrlContent + "!!!!!");
...
//                System.out.println(userContent.substring(userContent.lastIndexOf("获得")+3,userContent.indexOf("次感谢")-1));
        ...
        if (!StringUtils.isEmpty(sign) && sign.contains("公众号")) {
            user.setHavePublicAccount("有");
        } else {
            user.setHavePublicAccount("没有");
        }
        ...
        //看‘关注他’中有无关键字，判断性别
        String sexString = userUrlContent.select(".ProfileHeader-contentFooter").first().text();
...
//            ps.executeUpdate();
            ...
            appendMethodB("/home/dafei/niudeshuju.txt", jsonStr + "\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
        //把用户所关注的人的url加入到阻塞队列
